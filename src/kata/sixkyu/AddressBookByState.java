...
public class AddressBookByState {
   ...
   public static String byState(String str) {
      ...
      for (int i = 0; i < arr.length; i++) {
         ...
         String[] tempState = temp[2].trim().split("\\s+(?=\\S*+$)");
         String strFull = temp[0] + temp[1] + " " + tempState[0] +" "+ STATE_MAP.get(tempState[1]);
         
         if (map.containsKey(STATE_MAP.get(tempState[1]))) {
            ArrayList<String> list = map.get(STATE_MAP.get(tempState[1]));
            list.add(strFull);
            Collections.sort(list);
            map.put(STATE_MAP.get(tempState[1]), list);        
         }else {
